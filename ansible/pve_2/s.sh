...
# Roles to install via ansible-galaxy
- name: lae.proxmox
  src: lae.proxmox
- name: geerlingguy.ntp
EOF

# 4) collections requirements
...
[pve]
pve01 ansible_host=fd00:255::1
pve02 ansible_host=fd00:255::2
pve03 ansible_host=fd00:255::3

[all:vars]
ansible_user=root
EOF

# 6) group_vars/pve.yml
...
# 7) host_vars for each node
for node in pve01 pve02 pve03; do
  cat > host_vars/${node}.yml << 'EOF'
---
pve_ceph_osds:
  - /dev/nvme1n1
  - /dev/sda1
...
# 8) files/crushmap.txt
cat > files/crushmap.txt << 'EOF'
# begin crush map
...
# 9) templates/corosync.conf.j2
cat > templates/corosync.conf.j2 << 'EOF'
logging {
  debug: off
  to_syslog: yes
}
...
# 10) templates/ceph.conf.j2
cat > templates/ceph.conf.j2 << 'EOF'
[global]
  fsid = {{ lookup('env','CEPH_FSID') }}
  auth_client_required = cephx
  auth_cluster_required = cephx
  auth_service_required = cephx
  public_network = {{ pve_ceph_public_network }}
...
# 12) playbooks/tasks/main.yml
cat > playbooks/tasks/main.yml << 'EOF'
---
