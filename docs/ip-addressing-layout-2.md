...
| Function | IPv4 | IPv6 | Commentary |
|---------|------|------|------------|
| VM Data Plane | `10.<TID>.0.0/24` | `fd00:<TID>::/64` | Primary workload traffic |
| Anycast Gateway | `10.<TID>.0.254` | `fd00:<TID>::fffe` | Last-usable convention |
| Host Identities | `10.<TID>.255.0/24` | `fd00:<TID>:ff::/64` | Hypervisors in tenant VRF |
| VM Loopbacks | `10.<TID>.254.0/24` | `fd00:<TID>:fe::/64` | Workload routing identity |

**Commentary**

- `.254` / `::fffe` is a long-standing enterprise gateway convention
- `.255` / `:ff::` clearly signals infrastructure
- Loopbacks are routable identities, not access addresses
...
- Any PVE host can forward for the tenant
- Routing determines the active path
- VM mobility is transparent to the network
