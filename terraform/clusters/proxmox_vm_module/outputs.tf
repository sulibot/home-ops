locals {
  ansible_inventory_content = join("\n", concat(
    ...
    [for i in range(var.cp_quantity) : 
      format("${var.name_prefix}cp%02d            ansible_host=%s%d     ansible_user=root", i + 1, var.ipv6_address_prefix, i + var.cp_octet_start)
    ],
    [for i in range(var.wkr_quantity) : 
      format("${var.name_prefix}wk%02d            ansible_host=%s%d    ansible_user=root", i + 1, var.ipv6_address_prefix, i + var.wkr_octet_start)
    ],
    ["\n[controlplane]"],
    [for i in range(var.cp_quantity) : 
      format("${var.name_prefix}cp%02d            ansible_host=%s%d     ansible_user=root", i + 1, var.ipv6_address_prefix, i + var.cp_octet_start)
