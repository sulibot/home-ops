...
locals {
  ...
  # Handle the case where fabric is null
  _fabric_in   = try(var.sdn_controller.fabric, "")

  # Only trim if _fabric_in is not empty or null
  _fabric      = local._fabric_in != "" && local._fabric_in != null ? trimspace(local._fabric_in) : ""
  ...
  _mode_arg    = local._has_peers ? "-peers ${join(",", local._peers_list)}"
                : (local._has_fabric ? "-fabric ${local._fabric}" : "")
