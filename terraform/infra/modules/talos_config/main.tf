...
locals {
  ...
  controlplane_inline_manifests = var.apply_cilium_bgp_inline ? concat(
    # Optional: apply Cilium BGP configs from Talos inline manifests.
    # Default is off so Flux controls ordering after Cilium CRDs are available.
  ...
  registry_mirror_endpoints = var.registry_mirrors == null ? [] : (
    length(try(var.registry_mirrors.endpoints, [])) > 0 ?
    var.registry_mirrors.endpoints :
    [var.registry_mirrors.endpoint]
  )
  ...
  registry_mirror_patches = var.registry_mirrors != null && length(local.registry_mirror_endpoints) > 0 ? [
    yamlencode({
      machine = {
        registries = {
          mirrors = {
            ...
            registry => {
              endpoints    = local.registry_mirror_endpoints
              overridePath = true
            }
          }
        }
      ...
      machine = merge(
        {
          ...
          kubelet = {
            nodeIP = {
              # Use routable node interface IPs for kubelet node identity.
              # Regressions occur when loopback (dummy0 / fe::, .254) is selected here:
              # Cilium native routing and pod->service/API paths can break.
              validSubnets = [
                "${node.public_ipv6}/128",
                "${node.public_ipv4}/32",
              ]
            }
          }
