output "zone_id" {
  ...
  value       = proxmox_sdn_zone_evpn.main.id
}
...
output "ipv4_subnet_ids" {
  ...
  value       = { for k, v in proxmox_sdn_subnet.ipv4_subnets : k => v.id }
}

output "ula_subnet_ids" {
  description = "Map of VNet names to their ULA subnet IDs"
  value       = { for k, v in proxmox_sdn_subnet.ula_subnets : k => v.id }
}

output "gua_subnet_ids" {
  description = "Map of VNet names to their GUA subnet IDs"
  value       = { for k, v in proxmox_sdn_subnet.gua_subnets : k => v.id }
}
