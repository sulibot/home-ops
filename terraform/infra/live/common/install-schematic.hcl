...
locals {
  ...
  # v1.1.16: Fix namespace access - use nsenter instead of ip netns exec for accessing host namespace
  # v1.1.16: Fix syntax error in docker-start script (fi -> done for IPv6 loop)
  # v1.1.16: Implement network namespace isolation for FRR - resolves local address detection issues
  # v1.1.16: Remove update-source directives from Cilium neighbors to work around hostNetwork local address detection
  # v1.1.16: Fix loopback prefix-list syntax - add /32 and /128 prefix lengths to host addresses
  # v1.1.16: Replace MP-BGP with dual-stack traditional BGP - separate IPv4/IPv6 sessions for better Cilium compatibility
  # v1.1.16: Remove invalid passive directives from address-family blocks - passive is neighbor-level only
  # v1.1.16: Fix Cilium neighbor template - use bgp.cilium.peering.ipv6 config paths consistently
