...
resource "cloudflare_access_application" "wildcard" {
  account_id                = local.account_id
  name                      = "sulibot.com (*)"
  domain                    = "*.sulibot.com"
  type                      = "self_hosted"
  ...
  allowed_idps              = [cloudflare_access_identity_provider.authentik.id]
}

resource "cloudflare_access_policy" "wildcard_allow" {
  account_id     = local.account_id
  application_id = cloudflare_access_application.wildcard.id
...
resource "cloudflare_access_application" "authentik_bypass" {
  account_id       = local.account_id
...
resource "cloudflare_access_policy" "authentik_bypass" {
  account_id     = local.account_id
  application_id = cloudflare_access_application.authentik_bypass.id
  name           = "Bypass"
  decision       = "bypass"
  precedence     = 1
  include {
...
resource "cloudflare_access_application" "atuin_bypass" {
  account_id       = local.account_id
  name             = "Atuin (bypass)"
...
resource "cloudflare_access_policy" "atuin_bypass" {
  account_id     = local.account_id
  application_id = cloudflare_access_application.atuin_bypass.id
  name           = "Bypass"
  decision       = "bypass"
  precedence     = 1
  include {
...
output "wildcard_app_id" {
  value = cloudflare_access_application.wildcard.id
}

output "identity_provider_id" {
  value = cloudflare_access_identity_provider.authentik.id
}
EOF
}
