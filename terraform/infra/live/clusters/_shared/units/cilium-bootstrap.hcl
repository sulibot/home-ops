# Shared unit template: cilium-bootstrap
# Included by each cluster's cilium-bootstrap/terragrunt.hcl. All cluster-specific values
...
locals {
  ...
  exclude_unit = !local.cluster_enabled || (!local.bootstrap_mode && local.kubernetes_api_ready && local.cilium_daemonset_exists)
...
terraform {
  source = "${get_repo_root()}/terraform/infra/modules/cilium_bootstrap"
