# Shared unit template: compute
# Included by each cluster's compute/terragrunt.hcl. All cluster-specific values
...
locals {
  ...
  control_planes = [for i in range(local.cluster_config.controlplanes) : {
    ...
    swap_disk_gb  = try(local.control_plane_defaults.swap_disk_gb, 0)
  }]
  ...
  workers = [for i in range(local.cluster_config.workers) : {
    ...
    swap_disk_gb  = try(local.worker_defaults.swap_disk_gb, 0)
  }]
...
terraform {
  source = "${get_repo_root()}/terraform/infra/modules/cluster_core"
