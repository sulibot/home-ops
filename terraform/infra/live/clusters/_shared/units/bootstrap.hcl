# Shared unit template: bootstrap
# Included by each cluster's bootstrap/terragrunt.hcl. All cluster-specific values
...
dependencies {
  paths = ["${get_terragrunt_dir()}/../apply"]
}

dependency "talos_config" {
  config_path = "${get_terragrunt_dir()}/../config"
...
terraform {
  source = "${get_repo_root()}/terraform/infra/modules/talos_bootstrap"
  ...
  before_hook "prepare_output_directory" {
    ...
    execute = [
      "bash",
      "-lc",
      "mkdir -p '${get_repo_root()}/talos/clusters/cluster-${local.tenant_id}'"
    ]
  }
