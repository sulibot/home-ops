...
# NOTE: This section is now largely redundant due to health checks in apps.yaml
# The apps Kustomization waits for helm-controller to be healthy before creating
# HelmReleases, which prevents the observedGeneration: -1 race condition.
# Keeping this code commented for emergency fallback if needed.
...
echo "ℹ️  HelmRelease race condition prevention now handled by health checks in apps.yaml"
echo "   The apps Kustomization waits for helm-controller to be healthy before proceeding."
