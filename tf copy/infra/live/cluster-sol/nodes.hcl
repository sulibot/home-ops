locals {
  ...
  controlplane_nodes = [
    for i in range(local.cluster.locals.controlplanes) : {
      ...
      ipv6_gateway  = format("fd00:%d::fffe", local.cluster.locals.cluster_id)
      ipv4_gateway  = format("10.%d.0.254", local.cluster.locals.cluster_id)
      dns_servers   = [
        format("fd00:%d::fffe", local.cluster.locals.cluster_id),
        format("10.%d.0.254", local.cluster.locals.cluster_id)
      ]
    }
  ]
